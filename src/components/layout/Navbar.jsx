...
import adminAvatar from '../../assets/admin-avatar.png'; // Import ảnh đại diện
  ...
  return (
    <header className="bg-white shadow-sm sticky top-0 z-10">
      <div className="grid grid-cols-3 items-center max-w-7xl mx-auto px-4 py-3">
        ...
        {/* Middle: Giảm space-x-6 thành space-x-5 */}
        <nav className="justify-self-center hidden md:flex items-center space-x-5">
          {navItems.map(item => (
            <Link
              key={item.name}
              to={item.path}
